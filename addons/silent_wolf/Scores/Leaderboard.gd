...
func sort_by_score(a: Dictionary, b: Dictionary) -> bool:
	if a.score > b.score:  # Changed to sort descending (higher score at top)
		return true
	else:
		if a.score < b.score:
			return false
...
func add_item(player_name: String, score_value: String) -> void:
	...
	# Format score from MMSS to MM:SS
	var score_number = int(score_value)
	var minutes = score_number / 100
	var seconds = score_number % 100
	var formatted_time = "%02d:%02d" % [minutes, seconds]
	item.get_node("Score").text = formatted_time
